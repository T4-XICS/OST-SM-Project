...
# -----------------------------------------------------------------------------
# Prometheus Metrics (LSTM-VAE AE)
# -----------------------------------------------------------------------------
...
def start_prometheus_exporter():
    ...
    logger.info("Prometheus exporter started on 8001.")
...
# Evaluation helpers
# -----------------------------------------------------------------------------
def compute_scores(model, dataloader, device, percentile_threshold: float = 90.0):
    model.eval()
    scores = []
...
def check_and_reload_model():
    ...
    try:
        ...
        if last_modified_time is None or current_mtime > last_modified_time:
            logger.info(
                f"Model weights updated (mtime: {current_mtime}). Reloading model..."
            )
            load_model_safe()
        else:
...
def model_reload_worker(check_interval: int = 60):
...
def run_eval(batch_df, batch_id):
    ...
    with model_lock:
        if current_model is None:
            logger.warning(
                f"Batch {batch_id}: Model not loaded yet. Skipping evaluation."
            )
            return
    ...
    # ---- Prometheus AE metrics ----
    if scores:
        last_score = scores[-1]
        is_anomaly = 1 if len(anomaly_indices) > 0 else 0

        ae_last_anomaly_score.set(last_score)
        ae_threshold.set(threshold if threshold is not None else 0.0)
        ...
        try:
            ...
            for idx in anomalies:
                if idx < 0 or idx >= len(numeric_cols):
                    logger.warning(
                        f"Prometheus: Anomaly index {idx} out of range for sensors list."
                    )
                    continue
                ...
                if hasattr(tensor_seq, "shape") and len(tensor_seq.shape) == 2:
                    ...
                    if idx < num_sensors:
                        value = float(tensor_seq[-1, idx])
                        anomaly_gauge.labels(sensor=sensor).set(value)
                        logger.info(
                            f"Prometheus metric set: anomalous_sensor_event{{sensor='{sensor}'}} = {value}"
                        )
        except Exception as e:
            logger.error(f"Prometheus reporting failed: {e}")
    else:
        for sensor in numeric_cols:
            anomaly_gauge.labels(sensor=sensor).set(0.0)
